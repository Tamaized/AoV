	...
	public static void processPacket(ByteBuf parBB, Side parSide) throws IOException {
		...
		switch (getPacketTypeFromID(pktType)) {
			case MovingSound: {
				...
				if (entity != null)
					Minecraft.getMinecraft().getSoundHandler().playSound(new EntityMovingSound(SoundEvent.REGISTRY.getObjectById(bbis.readInt()), SoundCategory.PLAYERS, entity, false, 0));
